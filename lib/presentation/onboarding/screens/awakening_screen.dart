...
class _AwakeningScreenState extends ConsumerState<AwakeningScreen>
  ...
  final List<_Scene> _scenes = [
    ...
    _Scene(
      npc: 'O Vazio',
      text: '"Caelum só reage à disciplina.\n\nE sua forma sombria é moldada pelo que você faz no seu mundo."',
      mood: _Mood.identity,
      isVoid: true,
    ),
    _Scene(
  ...
  void initState() {
    ...
    _bgCtrl.forward().then((_) {
      _contentCtrl.forward().then((_) => setState(() => _canAdvance = true));
    });
  }
  ...
  Future<void> _finishOnboarding() async {
    ...
    if (_selectedHabit != null) {
      ...
      await ds.createInitialHabit(player.id, habitData.$1, habitData.$3);
    }
            ...
            child: Column(
              children: [
                    ...
                    child: FadeTransition(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            // NPC com balão de fala
                            if (scene.npc != null) _buildNpcBubble(scene),
                            if (scene.npc == null && !scene.isNarrativeChoice)
                              _buildNarrationText(scene.text),
                ...
                if (!scene.isHabitChoice)
                  _buildAdvanceButton(scene),

                if (scene.isHabitChoice && _selectedHabit != null)
                  _buildAdvanceButton(scene, label: 'Despertar'),
  ...
  // Balão de fala NPC — estilo Santuário
  Widget _buildNpcBubble(_Scene scene) {
    final isVoid = scene.isVoid;
    final color  = isVoid ? AppColors.purple : AppColors.gold;
    final icon   = isVoid ? Icons.blur_on : Icons.person_outline;
  ...
  Widget _buildAdvanceButton(_Scene scene, {String? label}) {
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            border: Border.all(
              color: _canAdvance ? AppColors.purple : AppColors.border,
              width: 1.5,
            ),
          ...
          child: Center(
            child: Text(
              label ?? (_currentPage < _scenes.length - 1 ? 'Continuar' : 'Despertar'),
              style: GoogleFonts.cinzelDecorative(
                fontSize: 14,
                color: _canAdvance
                    ? AppColors.textPrimary
