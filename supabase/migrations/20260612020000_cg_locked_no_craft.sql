-- ============================================================================
-- Card Game — REGRA (CEO 2026-06-12): carta BLOQUEADA (não-possuída) NÃO pode
-- ser criada do zero. Só dá pra criar CÓPIA de carta que o jogador JÁ possui —
-- preserva a economia de pacotes/coleção (obter primeiro, criar cópias depois).
--
-- 1) cg_create_card: rejeita se o jogador não possui a carta (reason 'locked').
-- 2) cg_card_info: o bloco `craft` só é exposto p/ cartas POSSUÍDAS (a UI deixa
--    de oferecer "CRIAR" do zero p/ bloqueadas).
-- Recria as duas funções (versão de 3 args, com p_concept) preservando o resto.
-- ============================================================================
...
begin
  ...
  -- REGRA: carta BLOQUEADA (não-possuída) não pode ser criada do zero.
  if not exists (select 1 from public.player_cards
                 where player_id = p_player and card_id = p_card_id) then
...
begin
  ...
  -- Craft (CRIAR CÓPIA) só p/ cartas POSSUÍDAS — bloqueada não cria do zero.
  if v_owned.copies is not null and v_card.rarity in ('comum','rara','epica','lendaria') then
